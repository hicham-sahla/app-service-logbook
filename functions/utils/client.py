...
class NotesClient:
    ...
    def inject(cls, func):
        ...
        def wrapper(context: CbcContext, *args ,**kwargs):
            ...
            client = cls(
                context.document_db_client, context.user.public_id, context.agent_or_asset.public_id
            )
    ...
    def get(self) -> Iterable[Note]:
        document = self.document_client.find_one(
            filter_map={'agent_or_asset_id': self.agent_or_asset_id}
        ) or {}
    ...
    def edit(self, text: str, note_id: str) -> Note | ErrorResponse:
        result = self.document_client.update_one(
            {'agent_or_asset_id': self.agent_or_asset_id, 'notes._id': ObjectId(note_id)},
            {'$set': {'notes.$.text': text}}
        )
    ...
    def remove(self, note_id: str, user_id: str) -> ErrorResponse | None:
        result = self.document_client.update_one(
            {'agent_or_asset_id': self.agent_or_asset_id, 'notes.user': user_id},
